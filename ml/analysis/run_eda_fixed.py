"""
EDA fixed - avoid pandas to_datetime crash on Python 3.14
"""
import sys
sys.path.insert(0, ".")
import json, math
...
timestamp_range = [df['ts'].min().isoformat().replace('+00:00','Z'), df['ts'].max().isoformat().replace('+00:00','Z')]
...
print(f"inside {inside_count} outside {outside_count}")
...
for cid, members in largest:
    ...
    largest_details.append({
        ...
        "date_span_days": int((sub['ts'].max() - sub['ts'].min()).days) +1,
...
for cid, members in clusters_500.items():
    sub = df.iloc[members].sort_values('ts')
    distinct_dates = int(sub['date'].nunique())
    span_days = int((sub['ts'].max() - sub['ts'].min()).days)
...
print(f"persist isolated {isolated} repeated {repeated} multi_day {multi_day}")
...
report = {
    ...
    "temporal": {
        ...
        "detections_by_hour": {int(k): int(v) for k,v in per_hour.to_dict().items()},
